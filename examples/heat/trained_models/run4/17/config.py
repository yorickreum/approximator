...
problem = Problem(
    ...
    [
        Constraint(identifier="initial condition", condition=lambda x, y: x == 0,
                   residual=lambda x, y, prediction: (prediction - (100 * torch.sin(math.pi * y / 40))) ** 2),
...
approximation_net = ApproximationNet(n_hidden_layers=4, n_neurons_per_layer=10)
...
def train_heat(pretrained_model=16):
...
def plot_heat(trained_model=15):
    dir_path = os.path.dirname(os.path.realpath(__file__))
