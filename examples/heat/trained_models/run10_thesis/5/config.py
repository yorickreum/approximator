...
approximation_net = ApproximationNet(n_hidden_layers=15, n_neurons_per_layer=40)
...
def train_heat(pretrained_model=None):
    ...
    print([str(approximation.use(0, z)) for z in range(11)])
...
def plot_heat_difference(trained_model=1):
    ...
    def analytically(t, z):
        return math.exp(-math.pi ** 2 * t / 100) * math.sin(math.pi * z / 10)

    plot_approximation_deviation(approximation, analytically, xlabel="t", ylabel="z", title="Difference between "
