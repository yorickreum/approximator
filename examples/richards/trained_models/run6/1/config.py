...
def plot_richards_difference(trained_model=1):
    load_trained_model(trained_model)

    dir_path = os.path.dirname(os.path.realpath(__file__))
    simpeg_results = pandas.read_csv(os.path.join(dir_path, f'./results_simpeg_lin_720s_t-z.csv'), index_col="t")
...
def plot_richards_celialike(trained_model=1, t=0.1, z_top=0.4, z_bottom=0):
    load_trained_model(trained_model)
    steps = 100
    import numpy as np
