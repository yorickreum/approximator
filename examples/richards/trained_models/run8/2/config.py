        ...
        [
            ...
            Constraint(identifier="upper boundary", condition=lambda x, y: y == domain.y_max and x > 0,
                       residual=lambda x, y, prediction: (prediction - (
                                   -.207 - .408 * torch.exp(-(x / 0.0005) ** 2))) ** 2),
...
approximation_net = ApproximationNet(n_hidden_layers=8, n_neurons_per_layer=20)
...
def train_richards(pretrained_model=None):
    ...
    approximation.train(
        learning_rate=1e-3,
        epochs=int(1e2),  # 1e4 boundaries 3e4 all 1e4 random steps
