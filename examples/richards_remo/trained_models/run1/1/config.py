...
def get_res_upper(x, y, prediction):
    ...
    remo_theta_1_tensor = torch.tensor(remo_theta_1(t_vals_cpu), device=approximator.DEVICE)  # , requires_grad=False
...
def get_res_lower(x, y, prediction):
    t_vals_cpu = x.cpu().tolist()
    remo_theta_3_tensor = torch.tensor(remo_theta_3(t_vals_cpu), device=approximator.DEVICE)  # , requires_grad=False
