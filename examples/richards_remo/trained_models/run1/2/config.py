...
from approximator.examples.richards_remo.func_lib import get_res, h_by_theta
...
domain = Domain(
    ...
    x_max=.55,
    y_min=0.254,  # y is spacial, z in m, positive/increasing upwards
    y_max=0.065
)
...
def get_res_upper(x, y, prediction):
    ...
    remo_theta_1_tensor = torch.tensor(remo_theta_1(t_vals_cpu), device=approximator.DEVICE, requires_grad=False)  # , requires_grad=False
    return prediction - h_by_theta(remo_theta_1_tensor)
    # return prediction - 0


def get_res_lower(x, y, prediction):
    t_vals_cpu = x.cpu().tolist()
    remo_theta_3_tensor = torch.tensor(remo_theta_3(t_vals_cpu), device=approximator.DEVICE, requires_grad=False)  # , requires_grad=False
    return prediction - h_by_theta(remo_theta_3_tensor)
    # return prediction - 0
