...
from approximator.examples.richards_remo.func_lib import get_res
...
def get_res_upper(x, y, prediction):
    ...
    remo_wsi_1_tensor = torch.tensor(remo_theta_1(t_vals_cpu), device=approximator.DEVICE)
    return prediction - remo_wsi_1_tensor


def get_res_lower(x, y, prediction):
    t_vals_cpu = x.cpu().tolist()
    remo_wsi_3_tensor = torch.tensor(remo_theta_3(t_vals_cpu), device=approximator.DEVICE)
    return prediction - remo_wsi_3_tensor
